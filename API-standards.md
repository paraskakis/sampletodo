...
3. Avoid unnecessary nesting for operations where it would be unnecessary - for example: `GET projects/{id}/tasks` and `POST projects/{id}/tasks` are good, but `PUT projects/{id}/tasks/{id}` or `DELETE projects/{id}/tasks/{id}` or even `GET projects/{id}/tasks/{id}` are unnecessarily complex.
